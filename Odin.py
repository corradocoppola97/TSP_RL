...
import math
import copy
from Thor import EnvType,EnvSpecs
import numpy as np
import time
...
class odin():
    def __init__(self, environment_specs,
        ...
        self._optspecs = optspecs
        self._model.set_optimizer(name=optimizer, options=optspecs)


        self._cmodel0 = copy.deepcopy(self._model.coremdl)
        ...
        if stop_function == None:
            self._stop_function = odin._no_stop
    ...
    def instance(self, st, action, st1 = None, t=None):
        if self._type == EnvType.minpath:
            if st1 is None:
                # print("st",st)
                st1, dummy1, dummy2, dummy3, dummy4 = self.env.response(st, action)
            return list(st.values()) + list(self.env.encode(action).values()) + list(st1.values()) + self.env.costlist() + ([] if t is None else [t])
        elif self._type == EnvType.min_path_compressed:
            if st1 is None:
                st1, dummy1, dummy2, dummy3, dummy4 = self.env.response(st, action)
            return list(st.values()) + list(st1.values()) + ([] if t is None else [t])
        else:
    ...
    def test(self, testcosts,  maximumiter=100):
        ...
        for episode in range(nepisodes):
            odecheck = False  # episode > odepact
            ...
            for t in range(maximumiter):
                ...
                # qvalues = [(self._model.coremdl(torch.as_tensor(self.instance(st, neigh))) +
                #             sum(mod0(torch.as_tensor(self.instance(st, neigh))) for mod0 in stabilizers)) / (len(stabilizers) + 1)
                #            for neigh in neighbors]
                if odecheck:
                    qvalues_md0 = [self._cmodel0(torch.as_tensor(self.instance(st, neigh))) for neigh in neighbors]
                at1 = (0, 0)
                if random.random() <= randomness:
                    at1 = neighbors[random.randint(0, len(neighbors) - 1)]
                else:
                    if odecheck:
                        at1 = self.env.Odin_action(qvalues, qvalues_md0, neighbors)
                    else:
    ...
    def solve(self, nepisodes = 100, noptsteps=1, randomness0 = 1, maximumiter=100, batchsize=5, display=(False,0), steps=3):
        ...
        for episode in range(nepisodes):
            ...
            for t in range(appliedmaximumiter):
                ...
                st1, rt, final, neighbors, feasible = self.env.response(st, at1)
                if odecheck:
                    qnext = max(self._model.coremdl(torch.as_tensor(self.instance(st1, at1))),
                            self._cmodel0(torch.as_tensor(self.instance(st1, at1))))
                else:
                    qnext = (sum(mod0(torch.as_tensor(self.instance(st1, at1))) for mod0 in stabilizers)) / max(len(stabilizers),1)
                # qnext = (self._model.coremdl(torch.as_tensor(self.instance(st1, at1)))+
                #            sum(mod0(torch.as_tensor(self.instance(st1, at1))) for mod0 in stabilizers))/(len(stabilizers)+1)
                #
                ...
                if not final:
                    ...
                    for f in range(steps):
                        qvaluesp = [self._model.coremdl(torch.as_tensor(self.instance(sp, neigh)))
                                    for neigh in neighborsp]
                        # qvaluesp = [(self._model.coremdl(torch.as_tensor(self.instance(sp, neigh))) +
                        #              sum(mod0(torch.as_tensor(self.instance(sp, neigh))) for mod0 in stabilizers)) / (len(stabilizers) + 1)
                        #             for neigh in neighborsp]
                        if odecheck:
                            qvaluesp_md0 = [self._cmodel0(torch.as_tensor(self.instance(sp, neigh))) for neigh in
                                    neighborsp]
                        if odecheck:
                            ap1 = self.env.Odin_action(qvaluesp, qvaluesp_md0, neighborsp)
                        else:
                        ...
                        if odecheck:
                            qnextp = max(self._model.coremdl(torch.as_tensor(self.instance(sp1, ap1))),
                                     self._cmodel0(torch.as_tensor(self.instance(sp1, ap1))))
                        else:
                            qnextp = sum(mod0(torch.as_tensor(self.instance(sp1, ap1))) for mod0 in stabilizers) / max(len(stabilizers),1)
                            #qnextp = (self._model.coremdl(torch.as_tensor(self.instance(sp1, ap1))) +
                                      # sum(mod0(torch.as_tensor(self.instance(sp1, ap1))) for mod0 in stabilizers)) / (len(stabilizers) + 1)
            ...
            stats.append(stat)
            # if RM > RM0:
            #     print("RM", RM, "RM0", RM0, "feasible", feasible)
            #     self._cmodel0 = copy.deepcopy(self._model.coremdl)
            #     RM0 = RM
            if nstabilizers>0:
            ...
            if len(variancereduction) < nvariancereduction :#and feasible and final:
                ...
                if len(pastvariance) < npastvariance:
                    ...
                    varvar = np.std(pastvariance)
                    if episode%10==0:
                        print("sigma",sigma,"avgvariance",avgvariance,"term",avgvariance + math.exp(-0.0001*episode)*varvar)
                    # if sigma >= avgvariance + math.exp(-0.0001*episode)*varvar:
                    #     scope = scope*theta
                    #     print("scope",scope)
                    #     if scope < 1e-10:
                    #         break
