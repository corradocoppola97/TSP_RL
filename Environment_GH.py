...
class tsp(environment):
    ...
    def __init__(self,envspecs):
        super(tsp,self).__init__(envspecs)
        self._edges = envspecs.get(EnvSpecs.edges)
        self._startingpoint = envspecs.get(EnvSpecs.startingpoint)
        self._finalpoint = envspecs.get(EnvSpecs.finalpoint)
        self._nodes = [i for i in range(self._startingpoint, self._finalpoint + 1)]
        self.nnodes = len(self._nodes)
        self.repetitions = len(self._costs)
        dic_mat = {}
        for rep in range(self.repetitions):
            dist_mat = [[0 for _ in range(self.nnodes+1)] for __ in range(self.nnodes+1)]
            costs_rep = self._costs[rep]
            for (i,j) in costs_rep:
                dist_mat[i][j] = costs_rep[(i,j)]

            for jj in range(1,self.nnodes):
                dist_mat[self.nnodes][jj] = costs_rep[(jj,0)]
                dist_mat[jj][self.nnodes] = costs_rep[(jj,0)]

            dic_mat[rep] = np.array(dist_mat)
        self.dic_mat = dic_mat
    ...
    def initial_state(self):
        return (self._startingpoint,np.ones((self.nnodes+1,self.nnodes+1),dtype=bool))


    def initial_mask(self,s0):
        m0 = [k for k in range(1,self.nnodes)]
        return m0
    ...
    def set_instance(self, rep, testcosts=None):
        ...
        if testcosts is None:
            ...
            self._linear_reward = {key: -testcosts[rep][key] + .0 for key in testcosts[rep].keys()}
            #self._costlist = list(testcosts[rep].values())
    ...
    def instances(self, st, mask):
        ...
        self._last_states = sts
        return insts

    def last_states(self):
        return self._last_states

    def output(self,st, at1, sts = None, insts = None):
        feasible=True
        if insts is not None:
            self._instances = insts
        if sts is not None:
        ...
        sts = self._last_states
        i,mat_st = st
        st1 = sts[at1]
        next,mat_st1 = st1
        rt = self._linear_reward.get((i,at1))
        if rt is None:
            rt = self._linear_reward.get((i,0))
        mask = []
        for k in range(1,self.nnodes):
            if mat_st1[at1,k]==1:
                if k!=at1:
                    mask.append(k)
        ...
        pre_final = mask == []
        final = pre_final and at1==self.nnodes
        if pre_final and not final:
            mask = [self.nnodes]
        ...
        return st1,rt,final,mask,feasible,insts[at1]
